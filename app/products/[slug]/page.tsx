...
export default async function ProductDetailPage({ params }: ProductPageProps) {
  ...
  const brandLabel = product.brands?.length === 1 ? "Merek tersedia" : product.brands && product.brands.length > 1 ? "Pilihan Multi-Brand" : "Katalog Resmi ASN";

  return (
    <main>
      {/* 1. Hero Dark Gradient (Exact Hikvision Standard Style) */}
          ...
          <div className="mt-9 grid gap-8 lg:grid-cols-[1fr_22rem] lg:items-end">
            ...
            <div className="rounded-3xl border border-white/15 bg-white p-6 shadow-2xl shadow-black/25">
              {product.brands && product.brands.length === 1 ? (
                ...
                <div>
                  <p className="mb-2 text-[10px] font-black tracking-wider text-slate-500 uppercase">Pilihan Brand Resmi</p>
              ...
              <div className="mt-4 flex items-start gap-3 rounded-2xl bg-slate-100 p-3.5 text-slate-700">
                ...
                <p className="text-xs leading-5">
                  Produk resmi pengadaan CV Agape Sinar Nirwana. Spesifikasi, sertifikasi uji (MTR/COC), dan ketersediaan unit dikonfirmasi dalam penawaran.
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="border-b border-slate-200 bg-white py-6 dark:border-slate-700 dark:bg-[#0d1a2c]" aria-label="Kategori dan standar sistem">
        <div className="site-container flex flex-wrap items-center gap-2">
          <span className="mr-2 text-xs font-bold text-slate-500 dark:text-slate-400">Pilihan Tipe & Standar:</span>
          ...
          {product.standards?.map((std) => (
            <span key={std} className="inline-flex items-center gap-1.5 rounded-full border border-cyan-200 bg-cyan-50 px-4 py-1.5 text-xs font-bold text-cyan-800 dark:border-cyan-800 dark:bg-cyan-950/50 dark:text-cyan-300">
      ...
      {/* 3. Verified Models Grid (Exact 3-Column Card Layout Across All Products) */}
      <section className="site-container py-14 sm:py-20" aria-labelledby="models-heading">
        <div className="flex flex-wrap items-end justify-between gap-5">
          <div>
            <p className="eyebrow">Pilihan spesifikasi terverifikasi</p>
            <h2 id="models-heading" className="section-title mt-2">Pilihan {product.name} untuk kebutuhan proyek</h2>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300">
              Berikut adalah tipe atau standar teknis riil yang siap diadakan oleh ASN. Jika part number atau dimensi spesifik proyek Anda belum tercantum, kirimkan detailnya melalui RFQ.
            </p>
          </div>
          <span className="rounded-full bg-cyan-500/10 px-4 py-2 text-xs font-black text-cyan-700 dark:text-cyan-300">
            {variants.length} tipe pilihan
          </span>
        </div>

        <div className="mt-9 grid gap-6 lg:grid-cols-3">
          {variants.map((variant, idx) => {
            const variantImg = variant.image || product.images[idx % product.images.length]?.src || product.images[0]?.src;
            return (
      ...
      {/* 4. Bottom RFQ Panel (Uniform Across All Pages) */}
      <section className="border-y border-slate-200 bg-white py-14 dark:border-slate-700 dark:bg-[#0d1a2c]">
        <div className="site-container">
          <div className="rfq-panel rounded-[2rem] px-6 py-12 text-center text-white sm:px-10">
            <p className="eyebrow !text-cyan-300">Tipe lain atau ukuran spesifik</p>
            <h2 className="mt-3 text-2xl font-black tracking-tight sm:text-4xl">Belum menemukan tipe yang dibutuhkan?</h2>
            <p className="mx-auto mt-4 max-w-2xl text-sm leading-6 text-slate-200">
              Kirim part number, dimensi teknis, gambar kerja, atau kebutuhan pengadaan proyek Anda. Tim ASN akan membantu menyiapkan opsi dan penawaran terbaik.
            </p>
            <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
              <a
                href={whatsappUrl(`Halo ASN, saya ingin konsultasi pengadaan ${product.name}. Berikut kebutuhan spesifikasi proyek saya: `)}
                target="_blank"
                rel="noreferrer"
