...
export default async function ProductDetailPage({ params }: ProductPageProps) {
  ...
  return (
    <>
      {/* 1. Hero Section (Exact Hikvision Dark Style) */}
          ...
          <div className="mt-9 grid gap-8 lg:grid-cols-[1fr_20rem] lg:items-end">
            <div>
              <span className="inline-flex rounded-full border border-cyan-300/25 bg-cyan-300/10 px-3 py-1.5 text-[10px] font-black tracking-[0.16em] text-cyan-200 uppercase">
                {product.brands && product.brands.length > 0 ? "Merek tersedia" : "Standar pengadaan resmi"}
              </span>
            ...
            <div className="rounded-3xl border border-white/15 bg-white p-7 shadow-2xl shadow-black/25">
              {product.brands && product.brands.length > 0 ? (
                ...
                <div className="flex items-center gap-3">
                  <div className="relative size-12 shrink-0 overflow-hidden rounded-xl bg-slate-100 p-2">
                    <Image src={mainImage.src} alt={product.name} fill className="object-contain p-1" />
                  </div>
                  <div>
                    <p className="text-xs font-black uppercase text-slate-900">{product.name}</p>
                    <p className="text-[10px] text-slate-500 font-semibold">{product.categoryLabel}</p>
                  </div>
                </div>
              )}
              <div className="mt-4 flex items-start gap-3 rounded-2xl bg-slate-100 p-4 text-slate-700">
                ...
                <p className="text-xs leading-5">
                  Produk yang dapat kami adakan. Ketersediaan model, spesifikasi, dan ketentuan garansi dikonfirmasi dalam penawaran.
                </p>
              </div>
            </div>
          </div>
      ...
      {/* 2. Sub-Category / Highlights Tag Bar */}
      <section className="border-b border-slate-200 bg-white py-6 dark:border-slate-700 dark:bg-[#0d1a2c]" aria-label="Pilihan tipe dan standar">
        <div className="site-container flex flex-wrap gap-2">
          ...
          {product.standards?.map((std) => (
            <span key={std} className="rounded-full border border-cyan-200 bg-cyan-50 px-4 py-2 text-xs font-bold text-cyan-800 dark:border-cyan-800 dark:bg-cyan-950/50 dark:text-cyan-300">
              {std}
            </span>
          ))}
        </div>
      ...
      {/* 3. Verified Models Grid (Exact Hikvision 3-Column Card Layout) */}
      <section className="site-container py-14 sm:py-20" aria-labelledby="models-heading">
        <div className="flex flex-wrap items-end justify-between gap-5">
          <div>
            <p className="eyebrow">Pilihan model & seri terverifikasi</p>
          ...
          <span className="rounded-full bg-cyan-500/10 px-4 py-2 text-xs font-black text-cyan-700 dark:text-cyan-300">
            {variants.length} model / seri pilihan
          </span>
        </div>

        <div className="mt-9 grid gap-6 lg:grid-cols-3">
          {variants.map((variant, idx) => {
            const variantImage = product.images[idx % product.images.length] || mainImage;
            return (
              <article key={variant.code} className="surface-card flex h-full flex-col overflow-hidden !p-0">
                ...
                <div className="flex flex-1 flex-col p-6">
                  ...
                  {variant.officialUrl && (
                    <a href={variant.officialUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-xs font-bold text-slate-500 transition hover:text-cyan-700 dark:text-slate-400 dark:hover:text-cyan-300">
                      Spesifikasi resmi <ExternalLink className="size-3.5" />
                    </a>
                  )}
                  <Link
                    href={`/contact?product=${encodeURIComponent(`${product.name} — ${variant.code}`)}&category=${product.category}`}
      ...
      <section className="border-y border-slate-200 bg-white py-14 dark:border-slate-700 dark:bg-[#0d1a2c]">
        <div className="site-container">
          <div className="rfq-panel rounded-[2rem] px-6 py-12 text-center text-white sm:px-10">
            <p className="eyebrow !text-cyan-300">Model lain atau spesifikasi khusus</p>
            ...
            <p className="mx-auto mt-4 max-w-2xl text-sm leading-6 text-slate-200">
              Kirim part number, spesifikasi teknis, atau gambar teknik Anda. Tim ASN akan membantu menyiapkan opsi dan penawaran harga resmi.
            </p>
