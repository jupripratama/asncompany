...
export function ProductCatalog() {
  ...
  const [category, setCategory] = useState<Filter>(
    categories.some((item) => item.id === initialCategory) ? (initialCategory as ProductCategory) : "all"
  );
  ...
  const categoryLabels: Record<string, { id: string; en: string }> = {
    all: { id: "Semua", en: "All" },
    mining: { id: "Mining Tools", en: "Mining Tools" },
    cctv: { id: "CCTV & Security", en: "CCTV & Security" },
    electrical: { id: "Electrical", en: "Electrical" },
    fasteners: { id: "Fasteners", en: "Fasteners" },
  };
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      const matchesQuery =
        !normalized ||
        `${product.name} ${product.description} ${product.highlights.join(" ")} ${(product.brands ?? []).join(" ")}`
  ...
  return (
    <>
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <label className="relative block w-full lg:max-w-sm">
          ...
          <input
            ...
            className="w-full rounded-full border border-slate-200 bg-white py-3 pr-4 pl-11 text-sm outline-none focus:border-cyan-500 dark:border-slate-700 dark:bg-slate-900"
          />
        </label>
          ...
          {[{ id: "all" }, ...categories].map((item) => {
            ...
            return (
              <button
                key={item.id}
                type="button"
                onClick={() => setCategory(item.id as Filter)}
                aria-pressed={category === item.id}
                className={cn(
                  "rounded-full px-4 py-2 text-xs font-bold transition",
                  category === item.id
                    ? "bg-cyan-600 text-white"
                    : "bg-slate-200/70 text-slate-600 hover:text-cyan-700 dark:bg-slate-800 dark:text-slate-300"
                )}
              >
                {label} ({count})
              </button>
      ...
      <p className="mt-5 text-sm text-slate-500" role="status">
        {filtered.length} {t("catalogProductsFound")}
      </p>

      {category === "cctv" && (
        <section
          className="surface-card mt-7 grid gap-6 p-6 lg:grid-cols-[0.8fr_1.2fr] lg:items-center lg:p-8"
          aria-labelledby="cctv-brand-heading"
        >
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-[10px] font-black tracking-[0.18em] text-cyan-700 uppercase dark:text-cyan-300">
                Multi-brand CCTV
              </p>
              <h2 id="cctv-brand-heading" className="mt-1 text-xl font-black tracking-tight text-slate-950 dark:text-white">
                {t("catalogMultiBrandHeading")}
              </h2>
              <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                {t("catalogMultiBrandDesc")}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
            {cctvBrands.map((brand) => (
              <BrandMark key={brand} brand={brand} />
            ))}
          </div>
        </section>
      )}
              ...
              <div className="flex flex-1 flex-col p-6">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-3">
                    ...
                    <h2 className="pt-1 text-lg font-extrabold text-slate-950 dark:text-white">
                      {product.name}
                    </h2>
                  </div>
                  {product.variants && (
                    <span className="shrink-0 rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                      {product.variants.length} {language === "en" ? "Types" : "Tipe"}
                    </span>
                  )}
                </div>
                ...
                <ul className="mt-4 space-y-1.5 text-xs text-slate-500 dark:text-slate-300">
                ...
                {product.brands && product.brands.length > 0 && (
                  <div className="mt-5 border-t border-slate-200 pt-4 dark:border-slate-800">
                    <p className="mb-2 text-[9px] font-black tracking-[0.16em] text-slate-400 uppercase">
                      {brandLabel}
                    </p>
                ...
                <div className="mt-6 flex items-center gap-2 border-t border-slate-200 pt-4 dark:border-slate-800">
                  ...
                  <Link
                    href={`/contact?product=${encodeURIComponent(product.name)}&category=${product.category}`}
                    className="button-primary flex-1 text-center text-xs"
                  >
                    {t("btnRequestQuote")}
                  </Link>
                </div>
      ...
      {filtered.length === 0 && (
        <div className="surface-card mt-8 p-12 text-center text-slate-500">
          <p>{t("catalogNotFound")}</p>
        </div>
      )}
    </>
  );
